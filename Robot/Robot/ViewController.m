...
- (UITextField *)nameTextField {
    if (!_nameTextField) {
        ...
        _nameTextField.backgroundColor = [UIColor systemGray6Color];
...
- (UITextView *)textView {
    if (!_textView) {
        ...
        _textView.backgroundColor = [UIColor systemBlueColor];
...
- (UIButton *)userDefaultsButton {
    if (!_userDefaultsButton) {
        ...
        [_userDefaultsButton setTitle:@"saveToUserDefaults" forState:UIControlStateNormal];
...
- (UIButton *)fileButton {
    if (!_fileButton) {
        ...
        [_fileButton setTitle:@"saveToFile" forState:UIControlStateNormal];
...
- (void)saveToUserDefaultAndReadToTextView {
    Robot *robot = [[Robot alloc]initWithName:self.nameTextField.text
                                  xCoord:[self.xCoordTextField.text doubleValue]
                                  yCoord:[self.yCoordTextField.text doubleValue]];
    [self saveToUserDefaults:robot];
    Robot *robotFromUserDefaults = [self getFromUserDefaults];
    self.textView.text = [NSString stringWithFormat:@"%@\nX-coordinate: %f\nY-coordinate: %f",
                          robotFromUserDefaults.name,
                          robotFromUserDefaults.xCoord,
                          robotFromUserDefaults.yCoord];
}
...
- (void)saveToFileAndReadToTextView {
    ...
    self.textView.text = [NSString stringWithFormat:@"%@\nX-coordinate: %f\nY-coordinate: %f",
                          robotFromFile.name,
                          robotFromFile.xCoord,
                          robotFromFile.yCoord];
}
